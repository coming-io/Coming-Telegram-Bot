...
* ✔️ Step 1:* Click *Join Airdrop* button and complete all mandatory tasks
* ✔️ Step 2:* Submit your details, including your wallet address
* ✔️ Step 3:* (optional): Increase your rewards by inviting your friends via your referral link
...
🟢 Join [Callicrypto Telegram group]({TELEGRAM_LINKS}) and [Callicrypto Telegram Channel]({TELEGRAM_CHANNEL}) (mandatory)
...
🔹 Join our community on Telegram and pass the verification process:
{TELEGRAM_LINKS}
"""

MAKE_SURE_TELEGRAM_CHANNEL = f"""

🔹 Follow our channel on Telegram in order to always keep yourself up-to-date with our latest news: 
{TELEGRAM_CHANNEL}
"""
...
Maximize your rewards! Share your unique referral link and stand a chance to earn even more. The top 25 referrals will receive a bonus of 5000 CALLI each!
REPLACEME
"""

WITHDRAWAL_TEXT = f"""
Withdrawals will be sent out automatically to your wallet address within 15 days after the launching date.
Note: Users found cheating would be disqualified and banned immediately."""
...
def submit_details(update, context):
    update.message.reply_text(text=PROCEED_MESSAGE, parse_mode=telegram.ParseMode.MARKDOWN, disable_web_page_preview=True)
...
def follow_twitter(update, context):
    ...
    update.message.reply_text(text="Type in the link to *your Twitter profile* to proceed.\n\nExample: \nhttps://twitter.com/thecallicrypto", parse_mode=telegram.ParseMode.MARKDOWN, reply_markup=ReplyKeyboardMarkup(
        [["Cancel"]]
    ))
